...
while True:
    ...
    if detection_active and len(border_polygon) >= 3:
        ...
        # Combined loop for persons, objects, and current classes
        ...
        elif person_inside and priority["warning"] > priority[max_state]:
            max_state = "warning"

        if initial_object_classes is None:
            initial_object_classes = set(current_object_classes)

        if set(current_object_classes) != initial_object_classes and priority["stolen"] > priority[max_state]:
            max_state = "stolen"
